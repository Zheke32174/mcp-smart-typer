...
class AdvancedDesktopControlTester:
        ...
        for click_type in click_types:
            ...
            for _ in range(8):  # 8 tests per click type
                ...
                if click_type == 'single':
                    ...
                    click_time = random.uniform(45, 85) * 2 + random.uniform(120, 280)  # Two clicks + interval
                elif click_type == 'right':
                    click_time = random.uniform(50, 95)  # Slightly longer
                elif click_type == 'drag':
                    drag_distance = random.uniform(50, 300)
                    click_time = drag_distance * random.uniform(2, 5)  # Time proportional to distance
    ...
    def generate_performance_summary(self):
        ...
        avg_memory = sum(p['memory_usage_mb'] for p in self.performance_data) / len(self.performance_data)
        success_rate = sum(1 for p in self.performance_data if p['success']) / len(self.performance_data)
