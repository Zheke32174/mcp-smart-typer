...
async def test_google_search():
    ...
    config = BrowserConfig(browser_type="chromium", headless=False, connect_to_existing=True)
    ...
    async with PlaywrightBrowserAutomation(config) as browser:
        ...
        if search_field:
            ...
            success = await browser.type_text(
                search_field.selector, "Playwright browser automation", human_like=True
            )
    ...
    async with PlaywrightBrowserAutomation(config) as browser:
        try:
            ...
            for i, field in enumerate(fields, 1):
                ...
                if field.bounds:
                    print(
                        f"   Position: ({field.bounds['x']:.0f}, {field.bounds['y']:.0f}) "
...
async def test_typing_demo():
    ...
    async with PlaywrightBrowserAutomation(config) as browser:
        ...
        await browser.navigate(
            "data:text/html,<html><body><h1>Typing Test</h1><input type='text' placeholder='Normal typing' id='normal'><br><br><input type='text' placeholder='Human-like typing' id='human'><br><br><textarea placeholder='Large text area' id='textarea'></textarea></body></html>"
        )
        ...
        await browser.type_text(
            "#human", "This is human-like typing with natural delays", human_like=True
        )
        await asyncio.sleep(1)
...
async def main():
    ...
    parser.add_argument(
        "--test", choices=["google", "typing", "analyze"], default="google", help="Test to run"
    )
