...
class WindowsUIAutomation:
                ...
                for pattern in browser_patterns:
                    if re.match(pattern, title, re.IGNORECASE):
                        windows.append({
                            ...
                            'handle': window._hWnd if hasattr(window, '_hWnd') else str(id(window)),
    ...
    async def detect_fields_in_window(self, window_info: Dict[str, Any], context_hint: str = "") -> List[Dict[str, Any]]:
    ...
    async def _detect_fields_via_ocr(self, screenshot: np.ndarray, window_info: Dict[str, Any], context_hint: str) -> List[Dict[str, Any]]:
        ...
        field_indicators = [
            ...
            "password", "pwd", "pass", "search", "enter text",
            "name", "address", "phone", "message", "comment"
        ]
    ...
    async def _detect_fields_via_patterns(self, screenshot: np.ndarray, window_info: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def _detect_browser_fields(self, window_info: Dict[str, Any], context_hint: str) -> List[Dict[str, Any]]:
    ...
    def _merge_similar_fields(self, fields: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for field in fields:
            ...
            for existing in merged:
                ...
                if (abs(field['bounds']['x'] - existing['bounds']['x']) < 20 and
                    abs(field['bounds']['y'] - existing['bounds']['y']) < 20):
    ...
    async def click_element(self, x: int, y: int, button: str = 'left', double_click: bool = False) -> Dict[str, Any]:
    ...
    async def type_text_at_position(self, x: int, y: int, text: str, delay: float = 0.05) -> Dict[str, Any]:
...
async def test_typing():
    ...
    result = await ui.type_text_at_position(x, y, "Hello, this is a test from MCP Smart Typer v2.0!")
