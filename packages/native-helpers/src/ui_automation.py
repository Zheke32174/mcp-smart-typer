...
class UIAutomationHandler:
    ...
    async def send_keys(self, keys: str, modifiers: List[str]) -> None:
        ...
        modifier_keys = {"ctrl": Key.ctrl, "alt": Key.alt, "shift": Key.shift, "win": Key.cmd}
        # Press modifiers
    ...
    async def detect_fields_with_fallback(
        self,
        context_hint: str,
        ...
        try:
            ...
            semantic_fields = await self._detect_semantic_fields(
                context_hint, window_title, include_hidden
            )
            ...
            if not semantic_fields or self._needs_fallback_analysis(semantic_fields, confidence):
                fallback_fields = await self._detect_fields_with_ocr_vision(
                    screenshot_path, semantic_fields
                )
            ...
            return {"fields": [], "error": str(e), "detection_method": "error"}

    async def _detect_semantic_fields(
        self, context_hint: str, window_title: Optional[str], include_hidden: bool
    ) -> List[Dict[str, Any]]:
    ...
    async def _detect_fields_with_ocr_vision(
        self, screenshot_path: str, semantic_fields: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            vision_results = vision_classifier.classify_multiple_fields(
                screenshot_path, field_bounds
            )
    ...
    async def _detect_field_regions_from_screenshot(
    ...
    def _determine_field_type(
        self, ocr_data: Dict[str, Any], vision_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _merge_field_detections(
        self, semantic_fields: List[Dict[str, Any]], fallback_fields: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
