...
class UIAutomationServiceImpl(ui_automation_pb2_grpc.UIAutomationServiceServicer):
    ...
    def TypeText(self, request, context):
        ...
        try:
            ...
            return ui_automation_pb2.TypeTextResponse(
                success=True, message="Text typed successfully"
            )
    ...
    def SendKeys(self, request, context):
        ...
        try:
            logger.info(
                f"SendKeys request: '{request.keys}' with modifiers {list(request.modifiers)}"
            )
            ...
            return ui_automation_pb2.SendKeysResponse(
                success=True, message="Keys sent successfully"
            )
    ...
    def GetActiveWindow(self, request, context):
        ...
        try:
            ...
            return ui_automation_pb2.GetActiveWindowResponse(
                success=True, window=window_info, message="Active window retrieved successfully"
            )
    ...
    def FindElement(self, request, context):
        ...
        try:
            logger.info(
                f"FindElement request: selector='{request.selector}', method='{request.method}'"
            )
            ...
            return ui_automation_pb2.FindElementResponse(
                success=False, message=f"FindElement failed: {str(e)}"
            )
    ...
    def ClickElement(self, request, context):
        ...
        try:
            logger.info(
                f"ClickElement request: ({request.x}, {request.y}), button='{request.button}'"
            )
            ...
            return ui_automation_pb2.ClickElementResponse(
                success=True, message=f"Clicked at ({request.x}, {request.y}) with {button} button"
            )

        except Exception as e:
            logger.error(f"ClickElement failed: {e}")
            return ui_automation_pb2.ClickElementResponse(
                success=False, message=f"Click failed: {str(e)}"
            )
    ...
    def EnumerateUIAElements(self, request, context):
        ...
        try:
            logger.info(
                f"EnumerateUIAElements request: window_title='{request.window_title}', types={list(request.element_types)}"
            )
            ...
            return ui_automation_pb2.EnumerateUIAElementsResponse(
                success=False, elements=[], message=f"Failed to enumerate UIA elements: {str(e)}"
            )
    ...
    def TypeIntoUIAElement(self, request, context):
        ...
        try:
            logger.info(
                f"TypeIntoUIAElement request: element_id='{request.element_id}', text='{request.text}'"
            )
        ...
        try:
            ...
            if not element_data:
                return ui_automation_pb2.GetElementDetailsResponse(
                    success=False, message=f"Element with ID '{request.element_id}' not found"
                )
            ...
            return ui_automation_pb2.GetElementDetailsResponse(
                success=True, element=uia_element, message="Element details retrieved successfully"
            )

        except Exception as e:
            logger.error(f"GetElementDetails failed: {e}")
            return ui_automation_pb2.GetElementDetailsResponse(
                success=False, message=f"Failed to get element details: {str(e)}"
            )
