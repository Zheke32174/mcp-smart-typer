        ...
        return {
            ...
            "cache_performance": {
                cache: {
                    ...
                    "hit_rate": (
                        self.cache_hits[cache] / (self.cache_hits[cache] + self.cache_misses[cache])
                        if (self.cache_hits[cache] + self.cache_misses[cache]) > 0
                        else 0
                    ),
                }
            ...
            "system_metrics": {
                "avg_memory_mb": (
                    round(sum(self.memory_usage) / len(self.memory_usage), 2)
                    if self.memory_usage
                    else 0
                ),
                "avg_cpu_percent": (
                    round(sum(self.cpu_usage) / len(self.cpu_usage), 2) if self.cpu_usage else 0
                ),
            },
        }
...
class OptimizedDeviceController:
    ...
    def __init__(self, cache_size: int = 10000, max_workers: int = 4):
        ...
        self.logger.info(
            "Optimized Device Controller initialized with advanced caching and monitoring"
        )
    ...
    def _execute_batch(self):
        ...
        self.logger.debug(
            f"Executed batch of {len(self._batch_operations)} operations in {duration:.3f}s"
        )
    ...
    def get_optimized_system_state(self) -> Dict:
        ...
        if (
            ...
            and current_time - self._device_cache[cache_key]["timestamp"] < 1.0
        ):
        ...
        with self._cache_lock:
            self._device_cache[cache_key] = {"data": system_state, "timestamp": current_time}
    ...
    def get_optimized_process_list(self, filter_system: bool = True) -> List[Dict]:
        ...
        if (
            cache_key in self._process_cache
            and current_time - self._process_cache[cache_key]["timestamp"] < 5.0
        ):
        ...
        # Cache the result
        with self._cache_lock:
            self._process_cache[cache_key] = {"data": processes, "timestamp": current_time}
    ...
    def _cleanup_caches(self):
        ...
        with self._cache_lock:
            ...
            old_keys = [
                key
                for key, value in self._process_cache.items()
        ...
        with self.batch_operations():
            for op in operations:
                try:
                    ...
                    results.append(
                        {"operation": op, "success": success, "timestamp": time.perf_counter()}
                    )
    ...
    def _get_optimization_recommendations(self) -> List[str]:
        ...
        for cache_name, stats in (
            self.metrics.get_performance_summary().get("cache_performance", {}).items()
        ):
            if stats["hit_rate"] < 0.7:
                recommendations.append(
                    f"Low cache hit rate for {cache_name}: {stats['hit_rate']:.2%}"
                )
        ...
        for op_name, stats in self.metrics.get_performance_summary().get("operations", {}).items():
            if stats["avg_time_ms"] > 50:  # Operations taking more than 50ms
                recommendations.append(
                    f"Slow operation {op_name}: {stats['avg_time_ms']:.1f}ms average"
                )
        ...
        if system_stats.get("avg_memory_mb", 0) > 1000:  # More than 1GB average
            recommendations.append(
                f"High memory usage: {system_stats['avg_memory_mb']:.1f}MB average"
            )

        if system_stats.get("avg_cpu_percent", 0) > 50:  # More than 50% CPU average
            recommendations.append(
                f"High CPU usage: {system_stats['avg_cpu_percent']:.1f}% average"
            )
