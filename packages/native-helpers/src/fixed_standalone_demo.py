...
class FixedStandaloneDemo:
    ...
    async def _test_mouse_performance(self):
        ...
        try:
            ...
            test_positions = [(100, 100), (200, 200), (300, 300), (400, 400), (150, 250)]
            ...
            self.logger.info(
                f"  📊 Mouse Results: {successful_moves}/{len(test_positions)} moves, {click_success}/{click_tests} clicks"
            )
    ...
    async def _test_keyboard_performance(self):
        ...
        try:
            # Test text typing
            test_texts = ["Hello World", "Testing 123", "Quick test", "Performance demo"]
            ...
            for i, text in enumerate(test_texts):
                ...
                self.logger.info(
                    f"  Text {i+1}: '{text}' → {chars_per_second:.1f} chars/sec - {'✅' if success else '❌'}"
                )
            ...
            for key_name, key_code in special_keys:
                ...
                self.logger.info(
                    f"  Key {key_name}: {duration*1000:.3f}ms - {'✅' if success else '❌'}"
                )
                await asyncio.sleep(0.1)
            ...
            typing_success_rate = sum(1 for r in typing_results if r["success"]) / len(
                typing_results
            )
            ...
            self.logger.info(
                f"  📊 Keyboard Results: {avg_chars_per_sec:.1f} avg chars/sec, {typing_success_rate:.1%} success rate"
            )
    ...
    async def _test_bulk_operations(self):
        ...
        try:
            ...
            self.logger.info(
                f"  🖱️ Bulk moves: {successful_bulk_moves}/{len(bulk_positions)} successful"
            )
            ...
            self.logger.info(
                f"  ⌨️ Bulk typing: {successful_bulk_types}/{len(bulk_texts)} successful"
            )
    ...
    async def _generate_final_report(self):
        ...
        try:
            ...
            summary = {
                ...
                "performance_summary": {
                    "mouse_avg_move_time_ms": self._get_metric(
                        "mouse_performance.avg_movement_time_ms", 0
                    ),
                    "keyboard_avg_chars_per_sec": self._get_metric(
                        "keyboard_performance.avg_chars_per_second", 0
                    ),
                    "bulk_moves_per_second": self._get_metric(
                        "bulk_operations.bulk_moves.moves_per_second", 0
                    ),
                    "bulk_chars_per_second": self._get_metric(
                        "bulk_operations.bulk_typing.chars_per_second", 0
                    ),
                },
                "component_metrics": {
            ...
            self.logger.info(
                f"  ⌨️ Typing Speed: {perf['keyboard_avg_chars_per_sec']:.1f} chars/sec"
            )
