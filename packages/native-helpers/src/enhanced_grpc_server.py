...
class FieldDetector:
            ...
            for i, contour in enumerate(contours):
                ...
                try:
                    ocr_text = pytesseract.image_to_string(
                        roi_enhanced, config="--psm 8 --oem 3"
                    ).strip()
    ...
    def _fields_overlap(
        self, field1: Dict[str, Any], field2: Dict[str, Any], threshold: float = 0.7
    ) -> bool:
        ...
        x_overlap = max(
            0, min(b1["x"] + b1["width"], b2["x"] + b2["width"]) - max(b1["x"], b2["x"])
        )
        y_overlap = max(
            0, min(b1["y"] + b1["height"], b2["y"] + b2["height"]) - max(b1["y"], b2["y"])
        )
...
class EnhancedUIAutomationService(ui_automation_pb2_grpc.UIAutomationServiceServicer):
    ...
    def _detect_fields_impl(self, request, context, use_fallback=True):
        ...
        try:
            logger.info(
                f"Field detection request: {request.context_hint} (fallback: {use_fallback})"
            )
            ...
            fields_data = self.field_detector.detect_fields(
                ...
                include_hidden=(
                    request.include_hidden if request.HasField("include_hidden") else False
                ),
            ...
            return ui_automation_pb2.DetectFieldsResponse(
                success=False, fields=[], message=f"Field detection failed: {str(e)}"
            )
    ...
    def TypeText(self, request, context):
        ...
        try:
            ...
            return ui_automation_pb2.TypeTextResponse(
                success=True, message=f"Successfully typed {len(request.text)} characters"
            )

        except Exception as e:
            logger.error(f"Typing failed: {e}")
            return ui_automation_pb2.TypeTextResponse(
                success=False, message=f"Typing failed: {str(e)}"
            )
        ...
        try:
            ...
            if not field_data:
                return ui_automation_pb2.GetFieldValueResponse(
                    success=False, message="Field not found in cache"
                )
            ...
            return ui_automation_pb2.GetFieldValueResponse(
                success=False, message=f"Get field value failed: {str(e)}"
            )
    ...
    def FocusField(self, request, context):
        ...
        try:
            ...
            return ui_automation_pb2.FocusFieldResponse(
                success=True, window_brought_to_front=True, message="Field focused successfully"
            )

        except Exception as e:
            logger.error(f"Focus field failed: {e}")
            return ui_automation_pb2.FocusFieldResponse(
                success=False, message=f"Focus field failed: {str(e)}"
            )
    ...
    def _get_ocr_field_value(self, field_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Perform OCR
            text = pytesseract.image_to_string(enhanced, config="--psm 8 --oem 3").strip()
