...
class OCRHandler:
    ...
    def preprocess_image(self, image: np.ndarray) -> np.ndarray:
        ...
        if height < 100 or width < 100:
            ...
            opening = cv2.resize(opening, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)
        
        return opening
    
    def extract_text(self, image_path: str, region: Optional[Tuple[int, int, int, int]] = None) -> Dict[str, Any]:
    ...
    def find_text_near_fields(self, image_path: str, field_bounds: List[Dict[str, int]]) -> List[Dict[str, Any]]:
    ...
    def analyze_field_labels(self, image_path: str, field_bounds: List[Dict[str, int]]) -> List[Dict[str, Any]]:
    ...
    def _classify_field_from_text(self, text: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ['comment', 'message', 'description', 'note', 'feedback', 'review']):
            return 'comment'
        ...
        # Name field patterns
        if any(keyword in text_lower for keyword in ['name', 'first name', 'last name', 'full name']):
            return 'name'
