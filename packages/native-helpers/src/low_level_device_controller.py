...
class LowLevelDeviceController:
    ...
    def _init_input_structures(self):
        ...
        class INPUT(ctypes.Structure):
            _fields_ = [("type", ctypes.wintypes.DWORD), ("input", INPUT_UNION)]
        ...
        try:
            ...
            return {
                ...
                "battery_percent": (
                    ...
                    if power_status.BatteryLifePercent <= 100
                    else None
                ),
                "battery_life_time": (
                    power_status.BatteryLifeTime
    ...
    def low_level_keyboard_control(self, action: str, **kwargs) -> bool:
        ...
        try:
            if action == "key_press":
                ...
                # Send key down
                result1 = self.user32.SendInput(
                    1, ctypes.byref(input_down), ctypes.sizeof(input_down)
                )
                time.sleep(duration)
    ...
    def start_performance_monitoring(
        self, callback: Callable[[SystemMetrics], None], interval: float = 1.0
    ):
            ...
            if action == "list_interfaces":
                ...
                for interface, addrs in psutil.net_if_addrs().items():
                    interface_info = {
                        "name": interface,
                        "addresses": [
                            {"family": addr.family.name, "address": addr.address} for addr in addrs
                        ],
                    }
