...
class UIAutomationServicer(ui_automation_pb2_grpc.UIAutomationServiceServicer):
    ...
    async def TypeText(
        self, 
        request: ui_automation_pb2.TypeTextRequest, 
    ...
    async def SendKeys(
        self, 
        request: ui_automation_pb2.SendKeysRequest, 
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.SendKeysResponse:
        """Send key combinations."""
        try:
            logger.info(f"SendKeys request: keys='{request.keys}', modifiers={list(request.modifiers)}")
        ...
        try:
            ...
            if window_info:
                ...
                return ui_automation_pb2.GetActiveWindowResponse(
                    success=True,
                    window=pb_window,
                    message="Active window information retrieved"
                )
            else:
                return ui_automation_pb2.GetActiveWindowResponse(
                    success=False,
    ...
    async def FindElement(
        self, 
        request: ui_automation_pb2.FindElementRequest, 
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.FindElementResponse:
        """Find an element on the screen."""
        try:
            logger.info(f"FindElement request: selector='{request.selector}', method='{request.method}', timeout={request.timeout}")
            ...
            element_info = await self.ui_handler.find_element(
                request.selector,
                request.method,
                timeout / 1000.0  # Convert to seconds
            )
    ...
    async def ClickElement(
        self, 
        request: ui_automation_pb2.ClickElementRequest, 
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.ClickElementResponse:
        """Click at specific coordinates."""
        try:
            logger.info(f"ClickElement request: x={request.x}, y={request.y}, button='{request.button}', double_click={request.double_click}")
