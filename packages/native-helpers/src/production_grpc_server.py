...
from unified_mcp_integration import UnifiedMCPSmartTyper, create_unified_mcp_smart_typer, FieldDetectionResult, AutomationJob
...
class MCPSmartTyperServicer(pb2_grpc.MCPSmartTyperServiceServicer):
    ...
    def _create_error_response(self, response_class, message: str, error_code: str = "INTERNAL_ERROR"):
    ...
    async def DetectFields(self, request, context):
        ...
        try:
            ...
            self.logger.info(f"Field detection completed: {len(fields)} fields found in {duration:.3f}s")
            return response
    ...
    async def TypeText(self, request, context):
        ...
        try:
            ...
            self.logger.info(f"Text typing requested for field {request.field_id} (length: {len(request.text)})")
            
            # Perform text typing
            start_time = time.perf_counter()
            success = await self.unified_typer.type_text(request.field_id, request.text, context_dict)
            duration = time.perf_counter() - start_time
    ...
    async def BulkOperations(self, request, context):
        ...
        try:
            ...
            self.logger.info(f"Bulk operations completed: {success_count}/{len(operations)} successful in {duration:.3f}s")
            return response
            ...
            return self._create_error_response(
                pb2.BulkOperationResponse, 
                f"Bulk operations failed: {str(e)}"
            )
    ...
    async def GetJobStatus(self, request, context):
        ...
        try:
            ...
            if job:
                ...
                if job.result:
                    response.result = json.dumps(job.result) if isinstance(job.result, (dict, list)) else str(job.result)
            else:
            ...
            return self._create_error_response(
                pb2.JobStatusResponse, 
                f"Get job status failed: {str(e)}"
            )
    ...
    async def GetPerformanceReport(self, request, context):
        ...
        try:
            ...
            return self._create_error_response(
                pb2.PerformanceReportResponse, 
                f"Performance report failed: {str(e)}"
            )
    ...
    async def HealthCheck(self, request, context):
        ...
        try:
            ...
            is_healthy = (
                self.unified_typer is not None and
                hasattr(self.unified_typer, 'device_controller') and
                self.unified_typer.device_controller is not None
            )
...
async def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('mcp_grpc_server.log'),
