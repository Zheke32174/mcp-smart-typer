...
class FieldPurposeClassifier:
    ...
    FIELD_TYPES = [
        'email', 'password', 'search', 'username', 'name', 'firstname', 'lastname',
        'phone', 'address', 'city', 'state', 'zipcode', 'country',
        'comment', 'message', 'description', 'text', 'number', 'date', 'time',
        'url', 'file', 'checkbox', 'radio', 'select', 'button', 'submit'
    ]
    
    def __init__(self, model_cache_dir: Optional[str] = None):
        """Initialize the field purpose classifier."""
        self.model_cache_dir = Path(model_cache_dir or 
                                  os.path.join(os.path.expanduser('~'), '.mcp-smart-typer', 'models'))
    ...
    def extract_features(self, field_info: FieldInfo) -> Dict[str, Any]:
        ...
        features['has_special_chars'] = 1 if any(not c.isalnum() and not c.isspace() for c in all_text) else 0
        ...
        input_types = ['text', 'email', 'password', 'search', 'tel', 'url', 'number', 'date', 'time']
        ...
        if field_info.autocomplete:
            ...
            common_autocomplete = ['name', 'email', 'username', 'password', 'tel', 'address', 'postal-code']
    ...
    def classify_field(self, field_info: FieldInfo) -> ClassificationResult:
        ...
        try:
            ...
            if self.is_trained and (self.lgb_model or self.gb_model):
                ...
                if self.lgb_model:
                    predictions = self.lgb_model.predict(features_df, num_iteration=self.lgb_model.best_iteration)
                    predicted_class = np.argmax(predictions[0])
                    confidence = float(np.max(predictions[0]))
                    
                    # Get feature importance
                    feature_importance = dict(zip(self.feature_names, self.lgb_model.feature_importance()))
                    ...
                    # Get feature importance
                    feature_importance = dict(zip(self.feature_names, self.gb_model.feature_importances_))
