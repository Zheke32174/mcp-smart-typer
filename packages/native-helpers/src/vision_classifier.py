...
class VisionFieldClassifier:
    ...
    FIELD_TYPES = ['email', 'password', 'search', 'comment', 'username', 'name', 'phone', 'text', 'button', 'checkbox']
    ...
    def __init__(self, model_cache_dir: str = None):
        ...
        """
        self.model_cache_dir = model_cache_dir or os.path.join(os.path.expanduser('~'), '.mcp-smart-typer', 'models')
    ...
    def _create_cnn_model(self) -> tf.keras.Model:
        ...
        model = tf.keras.Sequential([
            ...
            tf.keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=(*self.input_size, 3)),
    ...
    def _rule_based_classification(self, field_image: np.ndarray) -> Tuple[str, float]:
        ...
        gray = cv2.cvtColor(field_image, cv2.COLOR_BGR2GRAY) if len(field_image.shape) == 3 else field_image
        ...
        if len(contours) > 0:
            ...
            if contour_aspect_ratio > 2 and cv2.contourArea(largest_contour) > (width * height * 0.3):
                return 'button', 0.7
    ...
    def classify_multiple_fields(self, image_path: str, field_bounds_list: List[Dict[str, int]]) -> List[Dict[str, Any]]:
    ...
    def update_model_with_feedback(self, image_path: str, field_bounds: Dict[str, int], 
                                   correct_type: str, confidence_threshold: float = 0.8):
