...
class ExtendedUIAutomationServicer(UIAutomationServicer):
    ...
    async def _get_browser_instance(self, browser_type: str = "chromium") -> Optional[PlaywrightBrowserAutomation]:
    ...
    async def NavigateToBrowser(
        self,
        request: ui_automation_pb2.NavigateBrowserRequest,
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.NavigateBrowserResponse:
        """Navigate browser to a specific URL."""
        try:
            logger.info(f"NavigateToBrowser request: url='{request.url}', browser='{request.browser_type}'")
    ...
    async def AnalyzeBrowserDOM(
        self,
        request: ui_automation_pb2.AnalyzeDOMRequest,
        context: grpc.aio.ServicerContext
        ...
        try:
            ...
            if not browser:
                return ui_automation_pb2.AnalyzeDOMResponse(
                    success=False,
                    message=f"No active {browser_type} browser instance"
                )
            ...
            return ui_automation_pb2.AnalyzeDOMResponse(
                success=False,
                message=f"Failed to analyze DOM: {str(e)}"
            )
    
    async def TypeInBrowserField(
        self,
        request: ui_automation_pb2.TypeBrowserFieldRequest,
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.TypeBrowserFieldResponse:
        """Type text into a browser field with human-like delays."""
        try:
            logger.info(f"TypeInBrowserField request: field_selector='{request.field_selector}', "
            ...
            if success:
                return ui_automation_pb2.TypeBrowserFieldResponse(
                    success=True,
                    message=f"Successfully typed text into field"
                )
            else:
                return ui_automation_pb2.TypeBrowserFieldResponse(
                    success=False,
                    message="Failed to type text into field"
                )
            ...
            return ui_automation_pb2.TypeBrowserFieldResponse(
                success=False,
                message=f"Failed to type in browser field: {str(e)}"
            )
    
    async def CaptureScreenshot(
        self,
        request: ui_automation_pb2.ScreenshotRequest,
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.ScreenshotResponse:
        """Capture a screenshot of the browser page or specific element."""
        try:
            logger.info(f"CaptureScreenshot request: browser='{request.browser_type}', "
    ...
    async def GetBrowserPageInfo(
        self,
        request: ui_automation_pb2.GetPageInfoRequest,
        context: grpc.aio.ServicerContext
    ...
    async def ClickBrowserElement(
        ...
        try:
            logger.info(f"ClickBrowserElement request: selector='{request.selector}', "
            ...
            if success:
                ...
                return ui_automation_pb2.ClickBrowserElementResponse(
                    success=False,
                    message=f"Failed to click element: {request.selector}"
                )
            ...
            return ui_automation_pb2.ClickBrowserElementResponse(
                success=False,
                message=f"Failed to click browser element: {str(e)}"
            )
    
    async def WaitForBrowserElement(
        self,
        request: ui_automation_pb2.WaitForElementRequest,
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.WaitForElementResponse:
        """Wait for an element to appear in the browser."""
        try:
            logger.info(f"WaitForBrowserElement request: selector='{request.selector}', "
            ...
            if success:
                return ui_automation_pb2.WaitForElementResponse(
                    success=True,
                    message=f"Element appeared: {request.selector}"
                )
            else:
                return ui_automation_pb2.WaitForElementResponse(
                    success=False,
            ...
            return ui_automation_pb2.WaitForElementResponse(
                success=False,
                message=f"Failed to wait for browser element: {str(e)}"
            )
    
    async def ConfigureBrowser(
        self,
        request: ui_automation_pb2.ConfigureBrowserRequest,
        context: grpc.aio.ServicerContext
    ) -> ui_automation_pb2.ConfigureBrowserResponse:
        """Configure browser settings."""
        try:
            logger.info(f"ConfigureBrowser request: browser='{request.browser_type}', "
            ...
            return ui_automation_pb2.ConfigureBrowserResponse(
                success=True,
                message=f"Browser configuration updated for {browser_type}"
            )
            ...
            return ui_automation_pb2.ConfigureBrowserResponse(
                success=False,
                message=f"Failed to configure browser: {str(e)}"
            )
