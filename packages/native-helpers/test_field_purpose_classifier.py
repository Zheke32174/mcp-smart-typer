...
def test_email_fields():
    ...
    for i, case in enumerate(test_cases):
        ...
        print(f"  Top probabilities: {dict(sorted(result.all_probabilities.items(), key=lambda x: x[1], reverse=True)[:3])}")
        print()
...
def test_with_vision_predictions():
    ...
    print(f"  Vision says: {vision_prediction['predicted_type']} ({vision_prediction['confidence']:.3f})")
...
def test_batch_classification():
    ...
    for i, result in enumerate(results):
        print(f"  Field {i+1}: {result.predicted_type} (confidence: {result.confidence:.3f}, method: {result.method_used})")
    print()
